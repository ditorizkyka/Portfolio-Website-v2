...
export default function AdminSidebar({ onClose }: AdminSidebarProps) {
    ...
    return (
        <div className="flex h-full w-64 flex-col border-r border-[#1f1f1f] bg-black text-white">
            ...
            <nav className="flex-1 space-y-1 overflow-y-auto p-4 flex flex-col justify-between">
                ...
                <div className="pt-4 mt-4 border-t border-[#1f1f1f]">
                    <Link
                        href="/"
                        target="_blank"
                        className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-[#CCCCCC] hover:bg-[#1f1f1f] hover:text-[#6DD33D] transition-all duration-200 group"
                    >
                        <ExternalLink className="h-5 w-5 text-[#888888] group-hover:text-[#6DD33D]" />
                        Back to Website
                    </Link>
                </div>
            </nav>
